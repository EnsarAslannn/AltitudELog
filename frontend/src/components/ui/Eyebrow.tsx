  ...
  return (
    <div
      className={cn(
        ...
        tone === 'navy' ? 'text-primary' : 'text-on-primary/70',
        className,
      )}
      {...props}
    >
      <span className="h-1.5 w-1.5 shrink-0 rotate-45 bg-command" aria-hidden />
      <span className="eyebrow text-[11px] font-medium">{children}</span>
      {rule && (
        <span
          className={cn(
            'h-px flex-1',
            tone === 'navy' ? 'bg-outline-variant' : 'bg-on-primary/20',
          )}
          aria-hidden
        />
      )}
